...
const checkoutSource = readFileSync(new URL('../../../src/pages/matches/MatchCheckout.tsx', import.meta.url), 'utf8');
...
test('match checkout paints first and reconciles pending payments in background polling', () => {
  assert.match(apiSource, /getMatchDetail = \(token: string, matchId: number, reconcilePayments = false\)/);
  assert.match(checkoutSource, /getMatchDetail\(token, matchId, reconcilePayments\)/);
  assert.match(checkoutSource, /useEffect\(\(\) => \{ void loadMatch\(\); \}, \[bookingId, matchId, token\]\)/);
  assert.match(checkoutSource, /\(\) => loadMatch\(true\),\s*7_500/s);
  assert.match(detailSource, /getMatchDetail\(token!, matchId\)/);
});
