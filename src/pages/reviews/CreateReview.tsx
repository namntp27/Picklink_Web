...
export const CreateReview = () => {
  ...
  if (loading) return <div className="flex min-h-[70vh] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#477313]" /></div>;
  ...
  return <div className="min-h-dvh bg-[#f8fbf4] pt-[72px] text-[#0b2228]">
    ...
    <main className="mx-auto max-w-[980px] px-4 py-8">
      <section className="rounded-2xl border border-[#d8e4d4] bg-white p-6 shadow-sm">
        ...
        {booking && <div className="rounded-xl bg-[#edf5e9] p-4"><p className="text-[12px] font-bold uppercase text-[#477313]">{booking.bookingCode}</p><h2 className="mt-1 text-[21px] font-bold">{booking.venueName} · Sân {booking.courtNumber}</h2><p className="mt-1 text-[13px] text-[#64736a]">{booking.address}</p></div>}

        {review || alreadyReviewed ? <div className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 p-6 text-center"><CheckCircle2 className="mx-auto h-11 w-11 text-emerald-600" /><h2 className="mt-3 text-[19px] font-bold">Đã gửi đánh giá</h2><p className="mt-2 text-[14px] text-emerald-800">Booking này đã được đánh giá và không thể gửi thêm lần nữa.</p></div> : booking && !eligible ? <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-5"><h2 className="font-bold text-amber-900">Booking chưa đủ điều kiện đánh giá</h2><p className="mt-2 text-[14px] leading-6 text-amber-800">Chỉ cho đánh giá khi BookingStatus = Completed hoặc CheckInStatus = CheckedIn.</p></div> : booking && <div className="mt-6 space-y-6">
          ...
          <div><h2 className="text-[16px] font-bold">Nhận xét nhanh</h2><div className="mt-3 flex flex-wrap gap-2">{quickTags.map((tag) => <button className={`rounded-full border px-3 py-2 text-[13px] font-bold ${tags.includes(tag) ? 'border-[#477313] bg-[#477313] text-white' : 'border-[#d8e4d4] text-[#64736a]'}`} key={tag} onClick={() => toggleTag(tag)} type="button">{tag}</button>)}</div></div>

          <label className="block"><span className="text-[14px] font-bold">Nhận xét của bạn</span><textarea className="mt-2 min-h-32 w-full rounded-xl border border-[#d8e4d4] p-3 text-[14px] outline-none focus:border-[#477313]" maxLength={1000} onChange={(event) => setComment(event.target.value)} placeholder="Chia sẻ trải nghiệm thực tế tại sân..." value={comment} /></label>
          <label className="flex items-start gap-3 rounded-xl bg-[#edf5e9] p-4"><input checked={isAnonymous} className="mt-1 h-4 w-4 accent-[#477313]" onChange={(event) => setIsAnonymous(event.target.checked)} type="checkbox" /><span><strong className="text-[14px]">Ẩn tên công khai</strong><span className="mt-1 block text-[12px] text-[#64736a]">Hệ thống vẫn lưu tài khoản gửi để bảo đảm mỗi booking chỉ có một đánh giá.</span></span></label>
          <button className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0b2228] px-4 py-3.5 text-[15px] font-bold text-white disabled:opacity-50" disabled={submitting} onClick={() => void submit()} type="button">{submitting ? <Loader2 className="h-5 w-5 animate-spin" /> : <Star className="h-5 w-5 fill-current" />} Gửi đánh giá</button>
        </div>}
      </section>
