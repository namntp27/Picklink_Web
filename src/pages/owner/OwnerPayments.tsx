...
import { AlertCircle, CheckCircle2, Clock, Eye, Loader2, ReceiptText, RefreshCw, XCircle } from 'lucide-react';
...
export const OwnerPayments = () => {
  ...
  usePaymentRealtime((event) => {
    ...
    void getOperatorPayment(token, event.paymentId)
      .then((updated) => {
        setPayments((current) => {
          if (updated.paymentStatus !== 'WaitingForConfirmation') {
            return current.filter((item) => item.paymentId !== updated.paymentId);
          }
        ...
        setSelected((current) => current?.paymentId === updated.paymentId
          ? updated.paymentStatus === 'WaitingForConfirmation' ? updated : null
          : current);
      })
      .catch((requestError) => {
  ...
  const pendingPayments = useMemo(() => {
    return payments
      .filter((item) => item.paymentStatus === 'WaitingForConfirmation' && Boolean(item.receiptImageUrl))
      .sort((left, right) => {
  ...
  return <OwnerShell activeId="payments" innerClassName="max-w-[1320px]">
    <section className="owner-page-header flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="owner-kicker"><ReceiptText className="h-4 w-4" /> Hàng đợi đối soát</p><h1 className="mt-3 font-bold">Biên lai chờ xác nhận</h1><p className="mt-2 text-[14px] text-on-surface-variant">Chỉ hiển thị biên lai Player vừa gửi và đang chờ Owner xác nhận hoặc từ chối.</p></div>
      <div className="flex flex-wrap gap-2"><Link className="inline-flex items-center justify-center rounded-lg border border-outline-variant px-4 py-3 text-[14px] font-bold text-on-surface-variant" to="/owner/bookings">Xem toàn bộ đơn</Link><button className="inline-flex items-center justify-center gap-2 rounded-lg border border-primary px-4 py-3 text-[14px] font-bold text-primary" onClick={() => void load()} type="button"><RefreshCw className="h-4 w-4" /> Làm mới</button></div>
    </section>
    ...
    {selected && <div className="owner-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}><div className="owner-modal max-w-3xl">
