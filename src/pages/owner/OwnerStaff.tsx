  ...
  return (
    <OwnerShell activeId="staff">
      ...
      <section className="owner-panel">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left"><thead className="bg-surface-container-low text-[11px] uppercase text-on-surface-variant"><tr><th className="px-5 py-3">Booking</th><th className="px-5 py-3">Lịch sân</th><th className="px-5 py-3">Xác minh mã</th><th className="px-5 py-3">Thanh toán</th><th className="px-5 py-3">Kết quả</th></tr></thead><tbody className="divide-y divide-outline-variant">{history.map((item) => <tr key={item.bookingId}><td className="px-5 py-4"><p className="font-bold text-primary">{item.bookingCode}</p><p className="text-[12px] text-on-surface-variant">{item.playerName}</p></td><td className="px-5 py-4 text-[13px]"><p className="font-bold">{item.venueName} · Sân {item.courtNumber}</p><p className="text-on-surface-variant">{dateTime(item.startTime)}</p></td><td className="px-5 py-4 text-[12px]"><p className="font-bold">{item.codeVerifiedBy || 'Chưa có'}</p><p className="text-on-surface-variant">{dateTime(item.codeVerifiedAt)}</p></td><td className="px-5 py-4 text-[12px]"><p className="font-bold">{item.paymentConfirmedBy || 'Chưa có'}</p><p className="text-on-surface-variant">{dateTime(item.paymentConfirmedAt)}</p></td><td className="px-5 py-4 text-[12px]"><span className={`rounded-full px-2.5 py-1 font-bold ${item.checkInStatus === 'CheckedIn' ? 'bg-green-100 text-green-700' : item.checkInStatus === 'NoShow' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>{item.checkInStatus}</span><p className="mt-2 font-bold">{item.checkedInBy || item.noShowBy || 'Chưa có'}</p><p className="text-on-surface-variant">{dateTime(item.checkedInAt || item.noShowAt)}</p></td></tr>)}</tbody></table>{!history.length && !isLoading && <p className="p-8 text-center text-[13px] text-on-surface-variant"><Clock3 className="mx-auto mb-2 h-5 w-5" />Chưa có lịch sử phù hợp.</p>}</div>
