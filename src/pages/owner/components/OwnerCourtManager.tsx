...
type OptimisticUpdater = (updater: (current: OwnerVenue | undefined) => OwnerVenue) => void;

const CourtRow = ({ court, token, onChanged, onError, onOptimistic }: { court: OwnerCourt; token: string; onChanged: () => Promise<void>; onError: (message: string) => void; onOptimistic: OptimisticUpdater }) => {
  ...
  const save = async () => {
    setIsSaving(true);
    onError('');
    // Cập nhật lạc quan: đổi UI ngay, rồi mới gọi server và tự đồng bộ lại ở finally.
    onOptimistic((current) => ({ ...current!, courts: current!.courts.map((item) => item.courtId === court.courtId ? { ...item, ...draft } : item) }));
    try { await updateOwnerCourt(token, court.courtId, draft); }
    catch (error) { onError(error instanceof ApiError ? error.message : 'Không thể cập nhật sân con.'); }
    finally { await onChanged(); setIsSaving(false); }
  };
  ...
  const remove = async () => {
    ...
    try { await deleteOwnerCourt(token, court.courtId); }
    catch (error) { onError(error instanceof ApiError ? error.message : 'Không thể xóa sân con.'); }
...
export const OwnerCourtManager = ({ venue, token, onChanged, onError, onOptimistic }: { venue: OwnerVenue; token: string; onChanged: () => Promise<void>; onError: (message: string) => void; onOptimistic: OptimisticUpdater }) => {
  ...
  const addCourt = async () => {
    ...
    onError('');
    try {
      const created = await createOwnerCourt(token, venue.venueId, draft);
      onOptimistic((current) => ({ ...current!, courts: [...current!.courts, created] }));
    }
    catch (error) { onError(error instanceof ApiError ? error.message : 'Không thể thêm sân con.'); }
    finally { await onChanged(); setIsAdding(false); }
  };
  ...
  return (
    <section className="owner-panel p-5">
      ...
      <div className="space-y-3">
        ...
        {venue.courts.map((court) => <CourtRow court={court} key={court.courtId} onChanged={onChanged} onError={onError} onOptimistic={onOptimistic} token={token} />)}
        <div className="grid gap-3 rounded-lg border border-dashed border-primary/40 bg-primary/5 p-3 lg:grid-cols-[72px_1fr_1fr_140px_130px_150px_auto] lg:items-end">
          ...
          <select aria-label="Trạng thái sân con mới" className={`${inputClass} w-full`} onChange={(event) => setDraft({ ...draft, availabilityStatus: event.target.value as OwnerCourt['availabilityStatus'] })} value={draft.availabilityStatus === 'Inactive' ? 'Available' : draft.availabilityStatus}><option value="Available">Hoạt động</option><option value="Maintenance">Bảo trì</option></select>
