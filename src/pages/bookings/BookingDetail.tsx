      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-2xl border border-outline-variant bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 space-y-3">
            ...
            {booking.checkedInAt && <div className="flex gap-3"><span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><TicketCheck className="h-4 w-4" /></span><div><p className="text-[14px] font-bold">Đã check-in</p><p className="mt-0.5 text-[12px] text-on-surface-variant">{dateTime(booking.checkedInAt)} · Player đã check-in tại sân</p></div></div>}
            {booking.statusHistory.length === 0 && !booking.checkedInAt && <p className="text-[14px] text-on-surface-variant">Chưa có lịch sử trạng thái.</p>}
          </div>
        </section>
