            ...
            currentQueues.length === 0 ? (
              ...
              currentQueues.map((queue) => {
                ...
                <article
                  ...
                  style={{ background: queue.isPublic ? '#fbfdf8' : 'linear-gradient(135deg, #f6f9f4 0%, #edf2ea 100%)' }}
                >
                  <div className="flex flex-col gap-2.5">
                    <div className="min-w-0">
                      <div className="flex flex-wrap gap-1.5">
                        {!queue.isPublic && <span className={`community-badge !min-h-5 !px-1.5 !py-1 !text-[10px] ${
                          queue.isActive
                        ...
                        {queue.isPublic ? (
                          <>
                            <span className="community-badge !min-h-5 !px-1.5 !py-1 !text-[10px] text-[#526158]">Trình độ tối thiểu: {skillLevelName(queue.minSkillLevel)}</span>
                            <span className="community-badge !min-h-5 !px-1.5 !py-1 !text-[10px] text-[#526158]">Trình độ tối đa: {skillLevelName(queue.maxSkillLevel)}</span>
                          </>
                        ) : (
                          <span className="community-badge !min-h-5 !px-1.5 !py-1 !text-[10px] text-[#526158]">
                          ...
                          {host && (
                            <div className="mt-2.5 flex items-center gap-2">
                              <span className="grid h-6 w-6 shrink-0 place-items-center overflow-hidden rounded-lg border border-[#d8e4d4] bg-[#edf5e9] text-[10px] font-extrabold text-[#477313]">
                                {host.avatarUrl ? <img alt="" className="h-full w-full object-cover" decoding="async" loading="lazy" src={host.avatarUrl} /> : <span>{host.playerName.charAt(0).toUpperCase()}</span>}
                              </span>
                    ...
                    <div className="mt-2.5 divide-y divide-[#e2eae0] overflow-hidden rounded-lg border border-[#d8e4d4] bg-[#fbfcfa]">
                      {queue.isPublic ? (
                        <>
                          <div className="px-2.5 py-2"><p className="flex items-center gap-1.5 text-[10px] font-bold text-[#718077]"><MapPin className="h-3 w-3 text-[#477313]" />Khu vực</p><p className="mt-0.5 text-[11px] font-semibold leading-4 text-[#0b2228]">{queue.ward || 'Tự do'}, {queue.province || 'Toàn quốc'}</p><p className="mt-0.5 text-[10px] text-[#718077]">Bán kính {queue.searchRadiusKm} km</p></div>
                          <div className="px-2.5 py-2"><p className="flex items-center gap-1.5 text-[10px] font-bold text-[#718077]"><CalendarRange className="h-3 w-3 text-[#477313]" />Khoảng ngày</p><p className="mt-0.5 text-[11px] font-semibold leading-4 text-[#0b2228]">{queueDateRange(queue)}</p></div>
                          ...
                          <div className="px-2.5 py-2">
                            ...
                            <div className="mt-1.5 flex flex-wrap gap-1">
                              {approvedPlayers.map((player) => <span key={player.playerId} className="inline-flex items-center rounded border border-[#d8e4d4] bg-[#edf5e9] px-1.5 py-0.5 text-[10px] font-medium text-[#477313]">{player.playerName}</span>)}
                            </div>
                          </div>
                        </>
                      ) : (
                        <>
                          <div className="px-2.5 py-2">
                            <p className="flex items-center gap-1.5 text-[10px] font-bold text-[#718077]"><Clock className="h-3 w-3 text-[#477313]" />Khung giờ & Lịch hẹn</p>
                            <div className="mt-1 flex max-h-[80px] flex-col gap-0.5 overflow-y-auto">
                              {formatQueueSlots(queue.queueSlots, queue.replayType, false).map((slot, sIdx) => <span key={sIdx} className="text-[10px] font-semibold text-[#0b2228]">• {slot.dayLabel} · {slot.timeStart} - {slot.timeEnd}</span>)}
                            </div>
                          </div>
                          <div className="px-2.5 py-2">
                            <p className="flex items-center gap-1.5 text-[10px] font-bold text-[#718077]"><Users className="h-3 w-3 text-[#477313]" />Thành viên ({approvedPlayers.length})</p>
                            <div className="mt-1 flex flex-wrap gap-1">
                              {approvedPlayers.map((player) => <span key={player.playerId} className="inline-flex items-center gap-1 rounded border border-[#d2e4c8] bg-[#edf5e9] px-1.5 py-0.5 text-[9px] font-bold text-[#477313]">{player.playerName}{player.isHost && <span className="rounded bg-amber-100 px-0.5 text-[7px] text-amber-700">Host</span>}</span>)}
                            </div>
                          </div>
                        </>
                      )}
